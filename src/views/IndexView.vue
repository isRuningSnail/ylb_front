    ...
    <ul class="preferred-select clearfix" >
        ...
        <!-- <li>
            ...
            <div class="preferred-select-date">
                ...
                <div>
                    <span>余利可投资金额</span>
                    <p class="preferred-select-date-no"><b>{{ product.leftProductMoney }}</b>元</p>
                </div>
            </div>
            <p class="preferred-select-txt">
                优选计划项目，投资回报周期1个月，起点低，适合短期资金周转、对流动性要求高的投资人。
...
export default {
    ...
    mounted(){ //页面加载到浏览器，执行mounted
        ...
        //导入封装方法后   获取数据，更新页面
        doGet("/api/v1/plat/info",{}).then(resp => {
            console.log("平台信息："+resp);
            if(resp.data.code ==200){
                this.platInfo = resp.data.data;
            }
        ...
        doGet("/api/v1/product/index",{}).then(resp => {
            console.log("平台信息："+resp);
            if(resp.data.code ==200){
